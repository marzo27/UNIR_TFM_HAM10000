"""
╔══════════════════════════════════════════════════════════════════════╗
║   SVM CLASSIFIER — HAM10000 Skin Lesion Dataset (BALANCEADO)         ║
║   Versión: 2.0-balanceado  |  Clasificación BINARIA benigno/maligno  ║
║   Dataset: HAM10000_metadata_balanceo.csv (1954 + 1954, sin SMOTE)   ║
║   Genera: metricas.xlsx  con 7 hojas + 8 gráficos + JSON             ║
╚══════════════════════════════════════════════════════════════════════╝

Cambios respecto a la línea base (svm_ham10000_v1.py):
  1. Target binario: se clasifica benigno/maligno (categoria_dx),
     no las 7 clases dx originales. El dataset de entrada ya viene
     submuestreado y balanceado (1954 + 1954) según los criterios
     descritos en "Criterios_balanceo_dataset_TFM.docx".
  2. Sin SMOTE: al estar las clases ya balanceadas en origen, no se
     aplica sobremuestreo synthetic en el train. El paso 5 de la
     línea base se elimina por completo (pipeline de 9 a 8 pasos).
  3. Partición train/test agrupada por lesion_id (GroupShuffleSplit):
     las imágenes malignas conservan duplicados por lesión (1954
     imágenes de solo 1169 lesiones únicas); agrupar evita que dos
     fotos de la misma lesión queden repartidas entre train y test,
     tal como recomienda la sección 4 del documento de criterios.
...
Métricas capturadas (todas guardadas en metricas.xlsx):
  ...
  - Distribución de clases: original, train, test
...
Hojas Excel generadas:
  ...
  2. MÉTRICAS_POR_CLASE   — detalle por las 2 clases (Benigno/Maligno)
...
from sklearn.model_selection import (
    ...
    cross_val_score, GroupShuffleSplit
)
...
# ─────────────────────────────────────────────────────────────────────
BASE_PATH    = Path(r'D:\TFM\HAM10000')                       # raíz del dataset (imágenes)
CSV_BALANCEADO = BASE_PATH / 'HAM10000_metadata_balanceo.csv'  # dataset ya balanceado (1954+1954)
OUTPUT_DIR   = Path(r'D:\TFM\Resultados')                      # dónde se guardan los outputs
EXCEL_PATH   = OUTPUT_DIR / 'metricas_balanceado.xlsx'
JSON_PATH    = OUTPUT_DIR / 'metricas_balanceado.json'
MODEL_PATH   = OUTPUT_DIR / 'svm_model_balanceado.joblib'
PLOT_DIR     = OUTPUT_DIR / 'plots_svm_balanceado'
...
MODEL_NAME     = 'SVM_RBF_PCA95_Binario_Balanceado'

# Mapeo de las 7 clases dx originales a la variable binaria del TFM
MAPA_CATEGORIA = {
    'akiec': 'Maligno', 'bcc': 'Maligno', 'mel': 'Maligno',
    'bkl':   'Benigno', 'df':  'Benigno', 'nv':  'Benigno', 'vasc': 'Benigno',
}
...
# Nombres completos de las 2 clases (variable binaria del TFM)
CLASS_NAMES = {
    'Benigno': 'Benigno (bkl, df, nv, vasc)',
    'Maligno': 'Maligno (akiec, bcc, mel)',
}
...
CLASSES_BIN = list(CLASS_NAMES.keys())  # orden canónico; se ajusta al dataset real
...
# ═════════════════════════════════════════════════════════════════════
# PASO 1 — CARGA DEL DATASET BALANCEADO Y LOCALIZACIÓN DE IMÁGENES
# ═════════════════════════════════════════════════════════════════════
print("=" * 65)
print("  SVM CLASSIFIER — HAM10000 BALANCEADO  |  Binario benigno/maligno")
print(f"  Ejecución: {TIMESTAMP}")
print("=" * 65)
print("\n[1/8] Cargando dataset balanceado y localizando carpetas de imágenes...")

def localizar_carpetas_imagenes(base_path: Path):
    """Busca recursivamente las carpetas que contienen las imágenes .jpg."""
...
if not CSV_BALANCEADO.exists():
    raise FileNotFoundError(
        f"No se encontró el dataset balanceado en: {CSV_BALANCEADO}\n"
        "Ajusta la variable CSV_BALANCEADO al inicio del script."
    )

img_dirs = localizar_carpetas_imagenes(BASE_PATH)
# encoding='utf-8-sig': el CSV puede traer BOM (típico al guardar desde Excel
# en Windows); sin esto, la primera columna se lee como '\ufefflesion_id' y
# revienta más adelante con KeyError: 'lesion_id'.
df = pd.read_csv(CSV_BALANCEADO, sep=None, engine='python', encoding='utf-8-sig')
df.columns = df.columns.str.strip()  # por si quedan espacios extra en los encabezados
print(f"  ✓ Metadata : {CSV_BALANCEADO}")
print(f"  ✓ Filas    : {len(df)}  |  columnas: {list(df.columns)}")

# Recalcular la variable binaria objetivo a partir de dx
# (el CSV balanceado no incluye categoria_dx; se reconstruye con el
#  mismo mapeo documentado en Criterios_balanceo_dataset_TFM.docx)
df['categoria_dx'] = df['dx'].map(MAPA_CATEGORIA)
assert df['categoria_dx'].isna().sum() == 0, "Hay valores de dx sin mapear a categoria_dx"
print(f"  ✓ Distribución binaria: {df['categoria_dx'].value_counts().to_dict()}")


# ═════════════════════════════════════════════════════════════════════
# PASO 2 — CARGA Y APLANADO DE IMÁGENES
# ═════════════════════════════════════════════════════════════════════
print(f"\n[2/8] Cargando imágenes a {IMG_SIZE[0]}×{IMG_SIZE[1]} px (RGB)...")
...
y = df['categoria_dx'].values          # ← target BINARIO (no las 7 clases dx)
lesion_ids = df['lesion_id'].values    # ← necesario para la partición agrupada (Paso 3)

classes = sorted(np.unique(y))   # ['Benigno', 'Maligno']
...
# ═════════════════════════════════════════════════════════════════════
# PASO 3 — DIVISIÓN TRAIN / TEST AGRUPADA POR LESION_ID
# ═════════════════════════════════════════════════════════════════════
print(f"\n[3/8] Dividiendo datos por lesión (train {int((1-TEST_SIZE)*100)}% / "
      f"test {int(TEST_SIZE*100)}%, GroupShuffleSplit)...")

# Las imágenes malignas conservan duplicados por lesión (1954 imágenes de
# solo 1169 lesiones únicas; las benignas ya son 1 imagen por lesión tras
# el balanceo). Se agrupa por lesion_id para que ninguna lesión quede
# repartida entre train y test, evitando fuga de información.
gss = GroupShuffleSplit(n_splits=1, test_size=TEST_SIZE, random_state=RANDOM_STATE)
train_idx, test_idx = next(gss.split(X, y, groups=lesion_ids))
...
lesiones_train = set(lesion_ids[train_idx])
lesiones_test  = set(lesion_ids[test_idx])

print(f"  ✓ Train : {len(y_train)} muestras  |  {len(lesiones_train)} lesiones únicas")
print(f"  ✓ Test  : {len(y_test)}  muestras  |  {len(lesiones_test)} lesiones únicas")
print(f"  ✓ Distribución train: {dist_train_raw}")
print(f"  ✓ Distribución test : {dist_test_raw}")
assert lesiones_train.isdisjoint(lesiones_test), "¡Fuga de lesiones entre train y test!"
print("  ✓ Verificado: ninguna lesión aparece en ambos conjuntos")


# ═════════════════════════════════════════════════════════════════════
# PASO 4 — ESTANDARIZACIÓN + PCA
# ═════════════════════════════════════════════════════════════════════
print(f"\n[4/8] Escalando (StandardScaler) + PCA ({PCA_VARIANCE*100:.0f}% varianza)...")
...
# ═════════════════════════════════════════════════════════════════════
# PASO 5 — ENTRENAMIENTO SVM
# ═════════════════════════════════════════════════════════════════════
# NOTA: a diferencia de la línea base (v1), aquí NO se aplica SMOTE.
# El dataset ya llega balanceado 1954/1954 desde el submuestreo previo
# (ver Criterios_balanceo_dataset_TFM.docx), y tras la partición por
# lesión el train queda igualmente equilibrado (ver distribución
# impresa en el Paso 3). Introducir SMOTE encima de un dataset ya
# balanceado no aportaría nada y solo añadiría vectores sintéticos
# innecesarios.
print("\n[5/8] Entrenando SVM con kernel RBF (puede tardar varios minutos)...")
...
# ═════════════════════════════════════════════════════════════════════
print("\n[6/8] Evaluando modelo en test set...")
...
# --- AUC (one-vs-rest) ---
# NOTA: con solo 2 clases, sklearn.preprocessing.label_binarize colapsa
# el resultado a una sola columna (shape (n,1)), lo que rompe el resto
# del pipeline (que indexa y_bin[:, 0] y y_bin[:, 1]). Se construye el
# one-hot manualmente para que siempre tenga shape (n, n_classes).
y_bin = np.column_stack([(y_test == cls).astype(int) for cls in classes])
auc_per_class = {}
...
# ═════════════════════════════════════════════════════════════════════
# PASO 7 — VALIDACIÓN CRUZADA (sobre datos PCA)
# ═════════════════════════════════════════════════════════════════════
print(f"\n[7/8] Validación cruzada {N_CV_FOLDS}-fold (puede tardar)...")

# Igual que en la partición train/test, se agrupa por lesion_id para
# que ninguna lesión quede repartida entre folds distintos.
...
cv_acc = cross_val_score(_svm_cv, X_train_pca, y_train,
                         cv=skf, groups=lesion_ids_train, scoring='accuracy', n_jobs=-1)
cv_f1  = cross_val_score(_svm_cv, X_train_pca, y_train,
                         cv=skf, groups=lesion_ids_train, scoring='f1_macro',  n_jobs=-1)
...
# PASO 8 — GRÁFICOS
# ═════════════════════════════════════════════════════════════════════
print("\n[8/8] Generando gráficos y Excel...")
PLOT_DIR.mkdir(parents=True, exist_ok=True)


# ── Gráfico 1: Distribución de clases ──────────────────────────────
fig, axes = plt.subplots(1, 2, figsize=(12, 5))
fig.suptitle('Distribución de Clases — HAM10000 Balanceado', fontsize=14, fontweight='bold')
datasets = [
...
cfg_rows = [
    ...
    ("Lesiones únicas train",    len(lesiones_train)),
    ("Lesiones únicas test",     len(lesiones_test)),
...
title_row(ws5, 1, f"VALIDACIÓN CRUZADA {N_CV_FOLDS}-FOLD (sobre datos PCA, agrupada por lesión)", 1, 3)
...
sections = {
    ...
    "DATOS": [
        ("Dataset",                "HAM10000 balanceado (binario benigno/maligno)"),
        ("Total muestras",         str(len(df))),
        ("Clases",                 ', '.join(classes)),
        ("Num. clases",            str(n_classes)),
        ("Test split",             f"{TEST_SIZE*100:.0f}%"),
        ("Criterio de partición",  "GroupShuffleSplit agrupado por lesion_id"),
    ...
    "PREPROCESAMIENTO": [
        ...
        ("Balanceo de clases",     "Ninguno (dataset ya balanceado por submuestreo previo, ver Criterios_balanceo_dataset_TFM.docx)"),
    ],
    "TIEMPOS": [
...
baseline_json = {
    ...
    "timestamp"     : TIMESTAMP,
    "dataset"       : "HAM10000_balanceado",
    "img_size"      : list(IMG_SIZE),
    "n_train"       : int(len(y_train)),
    "n_test"        : int(len(y_test)),
    "n_lesiones_train": int(len(lesiones_train)),
    "n_lesiones_test"  : int(len(lesiones_test)),
